        ...
        while (time.time() - start_time) < DURATION:
            ...
            if data and len(data) >= 5:
                ...
                if result:
                    ...
                    position_mm = (
                        (x - 16220 / 2) / 27
                    ) / 1000  # (centered) encoder counts in m (approx)
                    log.append(
                        (t, position_mm, angle_rad, control)
                    )  # MAYBE write control/255. (normalize)?
                    print(
                        f"{t:.2f}s  x={position_mm:.4f} mm  θ={math.degrees(angle_rad):.2f}°  u={control}"
                    )
