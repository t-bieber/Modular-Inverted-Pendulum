...
def estimate_system_and_plot(csv_path: str, dt: float):
    ...
    velocity = savgol_filter(
        position, window_length=window, polyorder=poly, deriv=1, delta=dt
    )
    angular_velocity = savgol_filter(
        angle, window_length=window, polyorder=poly, deriv=1, delta=dt
    )

    # State-space data
    X_k = np.vstack(
        (position_smooth[:-1], velocity[:-1], angle_smooth[:-1], angular_velocity[:-1])
    )
    X_kplus1 = np.vstack(
        (position_smooth[1:], velocity[1:], angle_smooth[1:], angular_velocity[1:])
    )
    ...
    time = (
        ...
        else np.arange(len(position) - 1) * dt
    )
    ...
    np.savetxt(
        "A_matrix.csv", A, delimiter=",", header="Estimated A matrix", comments=""
    )
    np.savetxt(
        "B_matrix.csv", B, delimiter=",", header="Estimated B matrix", comments=""
    )
    np.savetxt(
        "K_matrix.csv", K, delimiter=",", header="LQR Gain matrix K", comments=""
    )
